...
public class PaymentServiceImpl implements PaymentService {
    ...
    private static final String SUCCESSFUL_URL =
            "/payments/success?sessionId={CHECKOUT_SESSION_ID}";
    ...
    public List<PaymentDto> getPaymentsByStatus(Long userId, String status, Pageable pageable) {
        return paymentRepository.findAllByStatus(Payment.PaymentStatus.valueOf(
                status.toUpperCase()))
                .stream()
                .map(paymentMapper::toDto)
                .toList();
